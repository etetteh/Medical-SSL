...
parser = argparse.ArgumentParser(description="Self-Supervised Learning with SIMCLR for Medical Imaging")
...
parser.add_argument('--ckpt_simclr', type=str, default='simclr_checkpoints', required=True, help='directory of checkpoints')
parser.add_argument('--ckpt_classifier', type=str, default='simclr_feat', required=True, help='directory of checkpoints')
...
simclr_train_dataset = lightly.data.LightlyDataset(
    input_dir=train_dir
    )
...
##### Load data for SIMCLR SSL ###########
simclr_train_dataloader = torch.utils.data.DataLoader(
...
class SIMCLRModel(pl.LightningModule):
    def __init__(self, backbone):
        ...
        # create a simclr based on model architecture
        self.simclr = \
            lightly.models.simclr.SimCLR(self.backbone, num_ftrs=args.num_ftrs,)
    ...
    def training_step(self, batch, batch_idx):
        (x0, x1), _, _ = batch
        y0, y1 = self.simclr(x0, x1)
        loss = self.criterion(y0, y1)
        self.log('train_loss', loss)
        return loss

    def configure_optimizers(self):
        optim = AdamP(self.simclr.parameters(), lr=1e-3, weight_decay=1e-4)
...
class Classifier(pl.LightningModule):
    def __init__(self, model):
        ...
        for p in self.simclr_model.parameters():  # reset requires_grad
    ...
    def forward(self, x):
        with torch.no_grad():
            y_hat = self.simclr_model.backbone(x).squeeze()
...
for model_name in model_zoo:
    ...
    print(f'\n Pre-training with self-supervised learning using SimCLR')
    
    model = SIMCLRModel(backbone = backbone)
    if args.jitter:
        dirpath_simclr = join(args.ckpt_simclr, Path(model_name+'_jitter'))
        checkpoint_simclr = ModelCheckpoint(
             monitor='train_loss',
             mode='min',
             save_top_k=1,
             save_last=True,
             dirpath=dirpath_simclr,
             filename=model_name)
        
        try:
            resume_simclr = join(dirpath_simclr, 'last.ckpt')
        except:
            print(f"No resume file")
            
        if os.path.exists(resume_simclr):
            simclr_trainer = pl.Trainer(max_epochs=args.epochs, 
                             gpus=gpus, gradient_clip_val=1.0, 
                             checkpoint_callback=True, 
                             auto_lr_find=True,
                             progress_bar_refresh_rate=2,
                             callbacks=[checkpoint_simclr,],
                             resume_from_checkpoint=resume_simclr)
            try:
                checkpoint = torch.load(sorted(glob(join(dirpath_simclr, model_name+'*.ckpt')))[-2])
            except:
                checkpoint = torch.load(join(dirpath_simclr, model_name+'.ckpt'))
        ...
        classifier = Classifier(model.simclr)
        ...
        simclr_train_dataloader.collate_fn=new_collate_fn
        
        dirpath_simclr = join(args.ckpt_simclr, Path(model_name+'_nojitter'))
        checkpoint_simclr = ModelCheckpoint(
             monitor='train_loss',
             mode='min',
             save_top_k=1,
...
if args.jitter:
    results_pickle = open(args.data_dir.strip('/')+'_simclr_jitter_results.pkl', 'wb')
else:
    results_pickle = open(args.data_dir.strip('/')+'_simclr_nojitter_results.pkl', 'wb')
