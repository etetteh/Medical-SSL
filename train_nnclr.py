...
parser = argparse.ArgumentParser(description="Self-Supervised Learning with NNCLR for Medical Imaging")
...
parser.add_argument('--ckpt_nnclr', type=str, default='nnclr_checkpoints', required=True, help='directory of checkpoints')
...
nnclr_train_dataset = lightly.data.LightlyDataset(
    input_dir=train_dir
    )
...
##### Load data for nnclr SSL ###########
nnclr_train_dataloader = torch.utils.data.DataLoader(
    nnclr_train_dataset,
...
class NNCLRModel(pl.LightningModule):
    def __init__(self, backbone):
        ...
        # create a nnclr based on model architecture
        self.nnclr = \
            lightly.models.nnclr.NNCLR(self.backbone, num_ftrs=args.num_ftrs)
        ...
        self.nn_replacer = modules.nn_memory_bank.NNMemoryBankModule()

    def training_step(self, batch, batch_idx):
        (x0, x1), _, _ = batch
        (z0, p0), (z1, p1) = self.nnclr(x0, x1)
    ...
    def configure_optimizers(self):
        optim = AdamP(self.nnclr.parameters(), lr=1e-3)
...
class Classifier(pl.LightningModule):
    def __init__(self, model):
        super().__init__()
        self.nnclr_model = model

        for p in self.nnclr_model.parameters():  # reset requires_grad
    ...
    def forward(self, x):
        with torch.no_grad():
            y_hat = self.nnclr_model.backbone(x).squeeze()
    ...
    def configure_optimizers(self):
        optimizer = AdamP(self.fc.parameters(), lr=1e-3)
...
for model_name in model_zoo:
    ...
    print(f'\n Pre-training with self-supervised learning using NNCLR')
    
    model = NNCLRModel(backbone = backbone)
    if args.jitter:
        dirpath_nnclr = join(args.ckpt_nnclr, Path(model_name+'_jitter'))
        checkpoint_nnclr = ModelCheckpoint(
             monitor='train_loss',
             mode='min',
             save_top_k=1,
             save_last=True,
             dirpath=dirpath_nnclr,
             filename=model_name)
        
        try:
            resume_nnclr = join(dirpath_nnclr, 'last.ckpt')
        except:
            print(f"No resume file")
            
        if os.path.exists(resume_nnclr):
            nnclr_trainer = pl.Trainer(max_epochs=args.epochs, 
                             gpus=gpus, gradient_clip_val=1.0, 
                             checkpoint_callback=True, 
                             auto_lr_find=True,
                             progress_bar_refresh_rate=2,
                             callbacks=[checkpoint_nnclr,],
                             resume_from_checkpoint=resume_nnclr)
            try:
                checkpoint = torch.load(sorted(glob(join(dirpath_nnclr, model_name+'*.ckpt')))[-2])
            except:
                checkpoint = torch.load(join(dirpath_nnclr, model_name+'.ckpt'))
        ...
        nnclr_trainer.fit(
                model,
                nnclr_train_dataloader
                )
        ###################################################################
        print(f'Training {model_name} classifier')
        classifier = Classifier(model.nnclr)
        ...
        nnclr_train_dataloader.collate_fn=new_collate_fn
        
        dirpath_nnclr = join(args.ckpt_nnclr, Path(model_name+'_nojitter'))
        checkpoint_nnclr = ModelCheckpoint(
             monitor='train_loss',
             mode='min',
             save_top_k=1,
...
if args.jitter:
    results_pickle = open(args.data_dir.strip('/')+'_nnclr_jitter_results.pkl', 'wb')
else:
    results_pickle = open(args.data_dir.strip('/')+'_nnclr_nojitter_results.pkl', 'wb')
