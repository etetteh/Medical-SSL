...
parser = argparse.ArgumentParser(description="Self-Supervised Learning with BYOL for Medical Imaging")
...
parser.add_argument('--ckpt_byol', type=str, default='byol_checkpoints', required=True, help='directory of checkpoints')
parser.add_argument('--ckpt_classifier', type=str, default='byol_feat', required=True, help='directory of checkpoints')
...
byol_train_dataset = lightly.data.LightlyDataset(
    input_dir=train_dir
    )
...
##### Load data for BYOL SSL ###########
byol_train_dataloader = torch.utils.data.DataLoader(
    byol_train_dataset,
...
class BYOLModel(pl.LightningModule):
    def __init__(self, backbone):
        ...
        # create a byol based on model architecture
        self.byol = \
            lightly.models.byol.BYOL(self.backbone, num_ftrs=args.num_ftrs,)
    ...
    def training_step(self, batch, batch_idx):
        (x0, x1), _, _ = batch
        y0, y1 = self.byol(x0, x1)
        loss = self.criterion(y0, y1)
        self.log('train_loss', loss)
        return loss

    def configure_optimizers(self):
        optim = AdamP(self.byol.parameters(), lr=1e-3, weight_decay=1e-4)
...
class Classifier(pl.LightningModule):
    def __init__(self, model):
        super().__init__()
        self.byol_model = model

        for p in self.byol_model.parameters():  # reset requires_grad
    ...
    def forward(self, x):
        with torch.no_grad():
            y_hat = self.byol_model.backbone(x).squeeze()
...
for model_name in model_zoo:
    ...
    print(f'\n Pre-training with self-supervised learning using BYOL')
    
    model = BYOLModel(backbone = backbone)
    if args.jitter:
        dirpath_byol = join(args.ckpt_byol, Path(model_name+'_jitter'))
        checkpoint_byol = ModelCheckpoint(
             monitor='train_loss',
             mode='min',
             save_top_k=1,
             save_last=True,
             dirpath=dirpath_byol,
             filename=model_name)
        
        try:
            resume_byol = join(dirpath_byol, 'last.ckpt')
        except:
            print(f"No resume file")
            
        if os.path.exists(resume_byol):
            byol_trainer = pl.Trainer(max_epochs=args.epochs, 
                             gpus=gpus, gradient_clip_val=1.0, 
                             checkpoint_callback=True, 
                             auto_lr_find=True,
                             progress_bar_refresh_rate=2,
                             callbacks=[checkpoint_byol,],
                             resume_from_checkpoint=resume_byol)
            try:
                checkpoint = torch.load(sorted(glob(join(dirpath_byol, model_name+'*.ckpt')))[-2])
            except:
                checkpoint = torch.load(join(dirpath_byol, model_name+'.ckpt'))
        ...
        byol_trainer.fit(
                model,
                byol_train_dataloader
                )
        ###################################################################
        print(f'Training {model_name} classifier')
        classifier = Classifier(model.byol)
        ...
        byol_train_dataloader.collate_fn=new_collate_fn
        
        dirpath_byol = join(args.ckpt_byol, Path(model_name+'_nojitter'))
        checkpoint_byol = ModelCheckpoint(
             monitor='train_loss',
             mode='min',
             save_top_k=1,
...
if args.jitter:
    results_pickle = open(args.data_dir.strip('/')+'_byol_jitter_results.pkl', 'wb')
else:
    results_pickle = open(args.data_dir.strip('/')+'_byol_nojitter_results.pkl', 'wb')
