...
parser = argparse.ArgumentParser(description="Self-Supervised Learning with BarlowTwins for Medical Imaging")
...
parser.add_argument('--ckpt_barlowtwins', type=str, default='barlowtwins_checkpoints', required=True, help='directory of checkpoints')
parser.add_argument('--ckpt_classifier', type=str, default='feat_checkpoints', required=True, help='directory of checkpoints')
...
parser.add_argument('--num_ftrs', type=int, default=2048, help='Number of hidden features dimension')
parser.add_argument('--mem_bank', type=int, default=4096, help='Memory Bank to store ')
...
parser.add_argument('--input_size', type=int, default=112, help='Input image resolution')
...
barlowtwins_train_dataset = lightly.data.LightlyDataset(
    input_dir=train_dir
    )
...
##### Load data for BarlowTwins SSL ###########
barlowtwins_train_dataloader = torch.utils.data.DataLoader(
    barlowtwins_train_dataset,
...
class BarlowTwinsModel(pl.LightningModule):
    def __init__(self, backbone):
        ...
        # create a barlowtwins based on model architecture
        self.barlowtwins = \
            lightly.models.barlowtwins.BarlowTwins(self.backbone, num_ftrs=args.num_ftrs)

        # create our loss with the optional memory bank
        self.criterion=lightly.loss.barlow_twins_loss.BarlowTwinsLoss(lambda_param=0.005)

    def training_step(self, batch, batch_idx):
        (x0, x1), _, _ = batch
        y0, y1 = self.barlowtwins(x0, x1)
        loss = self.criterion(y0, y1)
        self.log('train_loss', loss)
        return loss

    def configure_optimizers(self):
        optim = AdamP(self.barlowtwins.parameters(), lr=1e-3, weight_decay=1e-4)
...
class Classifier(pl.LightningModule):
    def __init__(self, model):
        super().__init__()
        self.barlowtwins_model = model

        for p in self.barlowtwins_model.parameters():  # reset requires_grad
    ...
    def forward(self, x):
        with torch.no_grad():
            y_hat = self.barlowtwins_model.backbone(x).squeeze()
...
for model_name in model_zoo:
    ...
    print(f'\n Pre-training with self-supervised learning using BarlowTwins')
    
    model = BarlowTwinsModel(backbone = backbone)
    if args.jitter:
        dirpath_barlowtwins = join(args.ckpt_barlowtwins, Path(model_name+'_jitter'))
        checkpoint_barlowtwins = ModelCheckpoint(
             monitor='train_loss',
             mode='min',
             save_top_k=1,
             save_last=True,
             dirpath=dirpath_barlowtwins,
             filename=model_name)
        
        try:
            resume_barlowtwins = join(dirpath_barlowtwins, 'last.ckpt')
        except:
            print(f"No resume file")
            
        if os.path.exists(resume_barlowtwins):
            barlowtwins_trainer = pl.Trainer(max_epochs=args.epochs, 
                             gpus=gpus, gradient_clip_val=1.0, 
                             checkpoint_callback=True, 
                             auto_lr_find=True,
                             progress_bar_refresh_rate=2,
                             callbacks=[checkpoint_barlowtwins,],
                             resume_from_checkpoint=resume_barlowtwins)
            try:
                checkpoint = torch.load(sorted(glob(join(dirpath_barlowtwins, model_name+'*.ckpt')))[-2])
            except:
                checkpoint = torch.load(join(dirpath_barlowtwins, model_name+'.ckpt'))
        ...
        barlowtwins_trainer.fit(
                model,
                barlowtwins_train_dataloader
                )
        ###################################################################
        print(f'Training {model_name} classifier')
        classifier = Classifier(model.barlowtwins)
        ...
        barlowtwins_train_dataloader.collate_fn=new_collate_fn
        
        dirpath_barlowtwins = join(args.ckpt_barlowtwins, Path(model_name+'_nojitter'))
        checkpoint_barlowtwins = ModelCheckpoint(
             monitor='train_loss',
             mode='min',
             save_top_k=1,
...
if args.jitter:
    results_pickle = open(args.data_dir.strip('/')+'_barlowtwins_jitter_results.pkl', 'wb')
else:
    results_pickle = open(args.data_dir.strip('/')+'_barlowtwins_nojitter_results.pkl', 'wb')
