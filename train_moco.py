...
parser = argparse.ArgumentParser(description="Self-Supervised Learning with MOCO for Medical Imaging")

parser.add_argument('--seed', type=int, default=0, help='seed for initializing training for reproducibility')
parser.add_argument('--data_dir', type=Path, default=None, required=True, help='directory of dataset')
parser.add_argument('--ckpt_moco', type=Path, default='moco_checkpoints', required=True, help='directory of checkpoints')
parser.add_argument('--ckpt_classifier', type=Path, default='moco_feat', required=True, help='directory of checkpoints')
...
parser.add_argument('--num_ftrs', type=int, default=32, help='Number of hidden features dimension')
...
moco_train_dataset = lightly.data.LightlyDataset(
    input_dir=train_dir
    )
...
##### Load data for MOCO SSL ###########
moco_train_dataloader = torch.utils.data.DataLoader(
    moco_train_dataset,
...
class MocoModel(pl.LightningModule):
    def __init__(self, backbone):
        ...
        # create a moco based on model architecture
        self.moco = \
            lightly.models.moco.MoCo(self.backbone, num_ftrs=args.num_ftrs)

        # create our loss with the optional memory bank
        self.criterion = lightly.loss.NTXentLoss(
            temperature=0.07,
            memory_bank_size=args.mem_bank)

    def training_step(self, batch, batch_idx):
        (x0, x1), _, _ = batch
        y0, y1 = self.moco(x0, x1)
        loss = self.criterion(y0, y1)
        self.log('train_loss', loss)
        return loss

    def configure_optimizers(self):
        optim = AdamP(self.moco.parameters(), lr=1e-3, weight_decay=1e-4)
...
class Classifier(pl.LightningModule):
    def __init__(self, model):
        ...
        for p in self.moco_model.parameters():  # reset requires_grad
    ...
    def forward(self, x):
        with torch.no_grad():
            y_hat = self.moco_model.backbone(x).squeeze()
...
for model_name in model_zoo:
    ...
    print(f'\n Pre-training with self-supervised learning using MOCO')
    
    model = MocoModel(backbone = backbone)
    if args.jitter:
        dirpath_moco = join(args.ckpt_moco, Path(model_name+'_jitter'))
        checkpoint_moco = ModelCheckpoint(
             monitor='train_loss',
             mode='min',
             save_top_k=1,
             save_last=True,
             dirpath=dirpath_moco,
             filename=model_name)
        
        try:
            resume_moco = join(dirpath_moco, 'last.ckpt')
        except:
            print(f"No resume file")
            
        if os.path.exists(resume_moco):
            moco_trainer = pl.Trainer(max_epochs=args.epochs, 
                             gpus=gpus, gradient_clip_val=1.0, 
                             checkpoint_callback=True, 
                             auto_lr_find=True,
                             progress_bar_refresh_rate=2,
                             callbacks=[checkpoint_moco,],
                             resume_from_checkpoint=resume_moco)
            try:
                checkpoint = torch.load(sorted(glob(join(dirpath_moco, model_name+'*.ckpt')))[-2])
            except:
                checkpoint = torch.load(join(dirpath_moco, model_name+'.ckpt'))
        ...
        moco_train_dataloader.collate_fn=new_collate_fn
        
        dirpath_moco = join(args.ckpt_moco, Path(model_name+'_nojitter'))
        checkpoint_moco = ModelCheckpoint(
             monitor='train_loss',
             mode='min',
             save_top_k=1,
...
if args.jitter:
    results_pickle = open(str(args.data_dir).strip('/') + '_moco_jitter_results.pkl', 'wb')
else:
    results_pickle = open(str(args.data_dir).strip('/')+'_moco_nojitter_results.pkl', 'wb')
